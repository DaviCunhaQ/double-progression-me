...
function App() {
  ...
  const [isCreatingExec, setIsCreatingExec] = useState<{id:number, state:boolean}|null>(null)
  ...
  function handleSubmitExec(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (execName === '' || execRestTime === '' || execSets === '') {
      ...
      const data = {
        ...
        setsData: Number(execSets) > 0 ? Array.from({length: Number(execSets)}, () => ({info: `ex:8 reps 100kg rir2`})) : []
      }
  ...
  return (
    <div className="flex flex-col items-center h-screen py-8">
      ...
      <button onClick={showFormTraining} className="p-2 bg-blue-500 text-white rounded-md cursor-pointer hover:bg-blue-600">
        Adicionar Treino
      </button>
      {isCreatingTraining && (
        <form onSubmit={handleSubmit} className="flex items-center mt-4 w-[21.875rem] gap-2 p-2 border border-black rounded-md">
          <div className="w-full flex flex-col gap-2 items-center justify-center">
            <input onChange={(e)=>{setTrainingName(e.target.value)}} type="text" placeholder="Nome do Treino" className="p-2 border border-black rounded-md w-full" />
            <input onChange={(e)=>{setRestTime(e.target.value)}} type="text" placeholder="Descanso entre exercícios" className="p-2 border border-black rounded-md w-full" />
          </div>
          <div className="flex flex-col gap-2 items-center justify-center">
            <button type="submit" className="p-2 bg-blue-500 text-white rounded-md cursor-pointer hover:bg-blue-600">
              <FaCheck />
            </button>
            <button type="button" onClick={()=>setIsCreatingTraining(false)} className="p-2 bg-gray-500 text-white rounded-md cursor-pointer hover:bg-gray-600">
              <MdCancel />
            </button>
          </div>
        </form>
      ...
      {trainingData.length > 0 ? (
        <>
          {trainingData.map((training: ITrainingData) => (
            <>
              <div className="flex justify-between items-center w-[21.875rem] p-2 border border-black rounded-t-md mt-4" key={training.id}>
                {isExecsFromTrainingShowing?.id === training.id && isExecsFromTrainingShowing.state ? (
                  <FaArrowUp onClick={()=>setIsExecsFromTrainingShowing({id: training.id, state: false})}/>
                ) : (
                  <FaArrowDown onClick={()=>setIsExecsFromTrainingShowing({id: training.id, state: true})}/>
                )}
                <div className="flex flex-col gap-2 items-center justify-center">
                  <h1 className="text-xl font-bold">{training.name.toUpperCase()}</h1>
                </div>
                <div className="flex flex-col gap-2 items-center justify-center">
                  <p>descanso:</p>
                  <p>{training.restTime}</p>
                </div>
                <FaTrashAlt className="bg-red" onClick={()=>{
                  const newTrainings = trainingData.filter((trainingData) => trainingData.id !== training.id)
                  const newExecs = execData.filter((execData) => execData.trainingId !== training.id)
                  setExecData(newExecs)
                  setTrainingData(newTrainings)
                }}/>
              </div>
              {isCreatingExec?.id === training.id && isCreatingExec.state ? (
                <form onSubmit={handleSubmitExec} className="flex items-center w-[21.875rem] gap-2 p-2 border border-black rounded-b-md">
                  <div className="w-full flex flex-col gap-2 items-center justify-center">
                    <input onChange={(e)=>{setExecName(e.target.value)}} type="text" placeholder="Nome do Exercício" className="p-2 border border-black rounded-md w-full" />
                    <input onChange={(e)=>{setExecRestTime(e.target.value)}} type="text" placeholder="Descanso entre séries" className="p-2 border border-black rounded-md w-full" />
                    <input className="border p-2 rounded-md w-full" placeholder="Séries" onChange={(e)=>{setExecSets(e.target.value)}} type="number"/>
                  </div>
                  <div className="flex flex-col gap-2 items-center justify-center">
                    <button onClick={()=>setTrainingIdToExec(training.id)} type="submit" className="p-2 bg-blue-500 text-white rounded-md cursor-pointer hover:bg-blue-600">
                      <FaCheck />
                    </button>
                    <button type="button" onClick={()=>setIsCreatingExec({id: training.id, state: false})} className="p-2 bg-gray-500 text-white rounded-md cursor-pointer hover:bg-gray-600">
                      <MdCancel />
                    </button>
                  </div>
                </form>
              ) : (
                <div onClick={()=>setIsCreatingExec({id: training.id, state: true})} className="flex gap-2 w-[21.875rem] p-2 rounded-b-md border border-black bg-blue-500 text-white justify-center items-center font-bold">
                    ...
                    execData.filter((exec) => exec.trainingId === training.id).map((exec) => (
                      <div className="flex flex-col gap-2 w-full" key={exec.id}>
                        ...
                        {isSetsFromExecShowing?.id === training.id && isSetsFromExecShowing.state ? (
                          ...
                          <p>{exec.rest}</p>
                          <FaTrashAlt onClick={()=>{
                            const newExecs = execData.filter((execData) => execData.id !== exec.id)
                            setExecData(newExecs)
                          }}/>
                        </div>
